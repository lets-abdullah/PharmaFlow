...
// Re-exports the Express app from server.ts as a Vercel handler
...
const DB_FILE = path.join('/tmp', 'db.json'); // Vercel only allows writes to /tmp
...
const isAdmin = (_req: any, _res: any, next: any) => {
  next();
};
...
// Medicine Routes
app.get('/api/medicines', authenticateToken, async (_req, res) => {
  const db = await getDB();
...
// Sales Routes
app.get('/api/sales', authenticateToken, async (_req, res) => {
  const db = await getDB();
  res.json(db.sales);
});
...
app.get('/api/reports/:type', authenticateToken, async (req, res) => {
  ...
  if (type === 'sales') {
    ...
    sendCsvReport(res, `sales-report-${generatedAt}.csv`, toCsv(['Sale ID', 'Date', 'Customer', 'Items', 'Sales Amount', 'Estimated Profit'], rows));
    return;
  }

  if (type === 'purchases') {
    const supplierById = new Map(db.suppliers.map(s => [s.id, s.name]));
    const medicineById = new Map(db.medicines.map(m => [m.id, m.name]));
    const rows = db.purchases
      ...
      .map(p => [
        ...
        medicineById.get(p.medicineId) || p.medicineId,
        supplierById.get(p.supplierId) || p.supplierId,
        p.quantity,
        p.purchasePrice.toFixed(2),
        p.totalPrice.toFixed(2),
      ]);
    sendCsvReport(res, `purchase-report-${generatedAt}.csv`, toCsv(['Purchase ID', 'Date', 'Medicine', 'Supplier', 'Quantity', 'Unit Cost', 'Total Cost'], rows));
    return;
  }
  ...
  if (type === 'profit') {
    ...
    sendCsvReport(res, `profit-report-${generatedAt}.csv`, toCsv(['Period', 'Sales Count', 'Sales Amount', 'Estimated Profit'], rows));
    return;
  }

  if (type === 'inventory') {
    const rows = db.medicines.map(m => [
      m.name, m.genericName, m.batchNumber, m.expiryDate,
      m.stock, m.minStock, m.purchasePrice.toFixed(2), m.salePrice.toFixed(2),
      (m.stock * m.purchasePrice).toFixed(2), (m.stock * m.salePrice).toFixed(2),
    ]);
    sendCsvReport(res, `inventory-report-${generatedAt}.csv`, toCsv(['Medicine', 'Generic Name', 'Batch', 'Expiry Date', 'Stock', 'Minimum Stock', 'Purchase Price', 'Sale Price', 'Stock Cost Value', 'Retail Value'], rows));
    return;
  }
